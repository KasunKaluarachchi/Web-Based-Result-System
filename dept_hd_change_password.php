      ...
      //echo  $old_pass;
	   ...
	   if($old_pass==$old)
	   {
           if($njew==$re_new)
           {
                ...
                echo "<script>alert('Successfully'); window.location='dept_hd_change_password.php'</script>";
           }
           else
           {
              echo "<script>alert('Your new and Retype Password is not match'); window.location='dept_hd_change_password.php'</script>";
           }
	   }
	   else
	   {
	   	  echo "<script>alert('Your old password is wrong'); window.location='dept_hd_change_password.php'</script>";
	   }
...
<![endif]-->
		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				$('#simple-table').css({'width':'200px', 'max-width': 'none'}).wrap('<div style="width: 100px;" />').parent().ace_scroll(
				  {
					horizontal: true,
